...
contract ICO is Ownable {
    ...
    modifier isNoContract() {
        require(
            ...
            "Contract is not allowed on ICO"
        );
        _;
    }
     ...
     */
    modifier isNotOver() {
        require(_isSale, "ICO is sale over");
        require(block.timestamp >= _startTime, "ICO is not started yet");
        require(block.timestamp <= _endTime, "ICO is already finished");
     ...
     */
    modifier isOver() {
        require(!_isSale || block.timestamp < _startTime || block.timestamp > _endTime || IERC20(_baseToken).balanceOf(address(this)) > _limitBaseAmount, "ICO is not finished yet");
        _;
    }
